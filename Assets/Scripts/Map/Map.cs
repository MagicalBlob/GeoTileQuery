...
public class Map
{
    ...
    public Map()
    {
        ...
        GameObject.AddComponent<BoxCollider>().size = new Vector3(25000, 0, 25000);
        ...
        string geoJsonTileServer = "https://tese.flamino.eu/api/tiles/";
        ...
        POIs.Add(new PointOfInterest("Castelo de S. Jorge", new Vector2D(38.713935, -9.133153)));
        ...
        POIs.Add(new PointOfInterest("Praça da Alegria", new Vector2D(38.717825, -9.145105)));
        POIs.Add(new PointOfInterest("Praça do Comércio", new Vector2D(38.706808, -9.136164)));
        POIs.Add(new PointOfInterest("Praça de Londres", new Vector2D(38.740842, -9.137563)));
    ...
    public void Start()
    {
        ...
        {
            Debug.Log($"[PERF] Map started at {DateTime.Now:HH:mm:ss.fff} ({DateTime.Now.Ticks / TimeSpan.TicksPerMillisecond} ms) | Zoom: {ZoomLevel} | Tile Load Distance: {TileLoadDistance} | Semaphore: {MainController.networkSemaphore.CurrentCount}");
        }
        Load();
    }
    ...
    private void Load()
    {
        ...
        if (MainController.DebugMetrics)
        {
            ...
            Debug.Log($"[PERF] Load called at {loadCalled:HH:mm:ss.fff} ({loadCalled.Ticks / TimeSpan.TicksPerMillisecond} ms)");
        }
        ...
        for (int tileY = originTile.y - TileLoadDistance.y; tileY <= originTile.y + TileLoadDistance.y; tileY++)
        {
            ...
            {
                if (!Tiles.TryGetValue($"{ZoomLevel}/{tileX}/{tileY}", out Tile existingTile))
                {
        ...
        if (MainController.DebugMetrics)
        {
            DateTime loadCallFinished = DateTime.Now;
            Debug.Log($"[PERF] Load area call finished at {loadCallFinished:HH:mm:ss.fff} ({loadCallFinished.Ticks / TimeSpan.TicksPerMillisecond} ms)");
        }
    }
